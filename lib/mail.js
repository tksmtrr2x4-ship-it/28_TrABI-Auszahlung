          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid ${border};">
            ...
            <tr>
              <td style="padding:32px;">
                ...
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid ${border};border-radius:8px;overflow:hidden;margin-bottom:16px;">
                  <tr>
                    <td style="padding:12px 16px;background:#e3f5f7;font-size:13px;color:${accentDark};font-weight:bold;width:50%;">Anteil am Gesamtvermögen (verbindlich)</td>
                    <td style="padding:12px 16px;font-size:15px;text-align:right;font-weight:bold;">${formatProzent(anteilProzent)} %</td>
                  </tr>
                  <tr>
                    <td style="padding:12px 16px;background:#e3f5f7;font-size:13px;color:${accentDark};font-weight:bold;border-top:1px solid ${border};">Voraussichtlicher Betrag (vorläufig)</td>
                ...
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#fff8e8;border:1px solid #f0dfb0;border-radius:8px;margin-bottom:20px;">
                  <tr>
                    <td style="padding:12px 16px;font-size:13px;line-height:1.6;color:#7a5a12;">
                      <strong>Wichtig:</strong> Verbindlich festgelegt ist ausschließlich dein
                      <strong>Anteil von ${formatProzent(anteilProzent)} %</strong>. Der ausgewiesene
                      Euro-Betrag ist eine aktuelle, <strong>vorläufige</strong> Schätzung auf Basis des
                      derzeitigen Kassenstands. Da die Stufenkasse aktuell noch belastet wird (z. B.
                      durch offene Rechnungen), kann der tatsächliche Auszahlungsbetrag bis zur
                      endgültigen Abrechnung noch <strong>steigen oder fallen</strong>.
                    </td>
                  </tr>
                </table>
...
function buildAbschlussHtml({ vorname, nachname, anteilProzent, betrag, erklaerungText, spendet }) {
  const accent = "#0e8c99";
  const accentDark = "#0b6d77";
  const border = "#d7e9ec";

  const erklaerungBlock =
    erklaerungText && erklaerungText.trim()
      ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#e3f5f7;border:1px solid ${border};border-radius:8px;margin-bottom:20px;">
          <tr>
            <td style="padding:14px 16px;font-size:13px;line-height:1.6;color:${accentDark};">
              <strong>Erläuterung:</strong><br/>
              ${erklaerungText.trim().replace(/\n/g, "<br/>")}
            </td>
          </tr>
        </table>`
      : "";
        ...
        <td align="center">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid ${border};">
            <tr>
              <td style="background:${accent};padding:24px 32px;">
                <span style="color:#ffffff;font-size:20px;font-weight:bold;letter-spacing:0.3px;">${ORG_NAME}</span><br/>
                <span style="color:#e3f5f7;font-size:13px;">Abschlussbestätigung – Auflösung der Stufenkasse</span>
              </td>
            </tr>
            <tr>
              <td style="padding:32px;">
                <p style="margin:0 0 16px;font-size:15px;">Hallo ${vorname},</p>
                <p style="margin:0 0 20px;font-size:15px;line-height:1.6;">
                  die Rückmeldefrist ist abgelaufen und alle offenen Posten der Stufenkasse sind
                  abgerechnet. Hier dein <strong>endgültiger</strong> Auszahlungsbetrag – die
                  ausführliche Bestätigung findest du auch im angehängten PDF.
                </p>
                ...
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid ${border};border-radius:8px;overflow:hidden;margin-bottom:16px;">
                  ...
                  <tr>
                    <td style="padding:12px 16px;background:#e3f5f7;font-size:13px;color:${accentDark};font-weight:bold;border-top:1px solid ${border};">${spendet ? "Gespendeter Betrag" : "Endgültiger Auszahlungsbetrag"}</td>
                    <td style="padding:12px 16px;font-size:15px;text-align:right;font-weight:bold;border-top:1px solid ${border};">${formatEuro(betrag)}</td>
                  </tr>
                  <tr>
                    <td style="padding:12px 16px;background:#e3f5f7;font-size:13px;color:${accentDark};font-weight:bold;border-top:1px solid ${border};">Name</td>
                ...
                <p style="margin:0 0 24px;font-size:14px;line-height:1.6;">
                  ${
                    spendet
                      ? "Dieser Betrag ist endgültig und ersetzt alle zuvor mitgeteilten, vorläufigen Schätzungen. Du hast angegeben, dass wir ihn stattdessen spenden – vielen Dank für deine Großzügigkeit!"
                      : "Dieser Betrag ist endgültig und ersetzt alle zuvor mitgeteilten, vorläufigen Schätzungen. Die Auszahlung erfolgt auf die von dir zuletzt angegebene IBAN."
                  }
                </p>
...
export async function sendAbschlussMail({
  to,
  vorname,
  nachname,
  anteilProzent,
  ...
  await getTransporter().sendMail({
    ...
    to,
    subject: `Deine endgültige Auszahlung – ${ORG_NAME}`,
    html: buildAbschlussHtml({ vorname, nachname, anteilProzent, betrag, erklaerungText, spendet }),
    attachments: [
      {
        filename: `Abschluss-${nachname}-${vorname}.pdf`,
        content: pdfBuffer,
