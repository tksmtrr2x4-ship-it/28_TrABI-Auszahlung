...
// Länderkennung aus der IBAN selbst ableiten (erste 2 Zeichen sind immer der
// ISO-Ländercode) statt sie zu erraten – pain.001.001.09 erwartet je nach
// Bank ein PstlAdr/Ctry für Dbtr und Cdtr.
function ctryAusIban(iban) {
  const normalisiert = normalizeIban(iban);
  return normalisiert.slice(0, 2) || "DE";
}
...
export function buildSepaXml({
  ...
  const debtorCtry = ctryAusIban(debtorIban);
  ...
  const transaktionen = zahlungen
    .map((z, i) => {
      ...
      const cdtrCtry = ctryAusIban(z.iban);
      ...
      // (nicht mit einem Platzhalter gefüllt) – viele Bank-Validatoren,
      // darunter Sparkassen, lehnen den "NOTPROVIDED"-Platzhalter hier ab.
  ...
  <CstmrCdtTrfInitn>
    ...
    <PmtInf>
      ...
      <Dbtr>
        <Nm>${xmlEscape(sepaSanitize(debtorName))}</Nm>
        <PstlAdr>
          <Ctry>${xmlEscape(debtorCtry)}</Ctry>
        </PstlAdr>
      </Dbtr>
      <DbtrAcct>
