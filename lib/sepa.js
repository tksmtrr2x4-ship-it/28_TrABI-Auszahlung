...
// SEPA-Regelwerk: Beträge müssen mindestens 0,01 € betragen – ein
// Nullbetrag ist keine gültige Überweisung und lässt sonst die ganze
// Sammelüberweisung als "ungültig" scheitern.
const MINDESTBETRAG = 0.01;
...
export function buildZahlungenAusAntworten(
  ...
  for (const a of antworten) {
    ...
    const betragGerundet = Number(betrag.toFixed(2));

    if (betragGerundet < MINDESTBETRAG) {
      ausgelassen.push({
        name,
        grund: `Betrag ${betragGerundet.toFixed(2)} € liegt unter dem SEPA-Mindestbetrag von 0,01 €`,
      });
      continue;
    }
...
export function buildSepaXml({
  ...
  // Wichtig: erst jeden Einzelbetrag auf 2 Nachkommastellen runden und DANN
  // aufsummieren – nicht die ungerundeten Rohbeträge aufsummieren und erst
  // am Ende runden. Sonst weicht CtrlSum von der Summe der tatsächlich im
  // Dokument stehenden Einzelbeträge ab (Rundungsfehler über viele
  // Transaktionen), was SEPA-Validatoren zuverlässig als ungültig ablehnen.
  const ctrlSum = formatBetrag(
    zahlungen.reduce((s, z) => s + Number(formatBetrag(z.betrag)), 0)
  );
